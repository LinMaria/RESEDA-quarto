...
::: entry-header
# R Crash Course Part I {#r-crash-course-part-i .entry-title}
:::
...
Your can install new packages to your library by clicking on the **Packages**-tab and then **Install** in the Files Pane of R-Studio. Anyway, download and installation of packages can also be done script-based via the function [install.packages()]{.crayon-inline} (illustrated using the [ggplot2](https://cran.r-project.org/web/packages/ggplot2/ggplot2.pdf) package, which is useful for creating elegant data visualisations):
...
You can list all packages, wich are loaded in your current R session by using [search()]{.crayon-inline}. Activated packages also have a tick symbol next to their names in the Files pane (Packages tab) of R-Studio.
...
You will see a number in square brackets [$$1$$]{.crayon-inline} at the beginning of your output prompts. This number refers to the length of your output, i.e., the number of elements, which is 1 for all examples above. More on that in chapter [Vectors](https://blogs.fu-berlin.de/reseda/r-crash-course/#ch5). In addition to these standard operators, there were plenty of other operators commonly encountered in R:
...
|      [  []  $$\[$$]  \$  \@  ]{.crayon-inline .theme:amityresedaterminal}      | indexing in vectors, matrices, and data frames                       |
